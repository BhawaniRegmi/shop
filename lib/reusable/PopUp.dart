        ...
        child: Container(
          child: new Column(
            ...
            children: <Widget>[
              ...
              if (widget.newStatus == "Rescheduled") dropdownExample


                else if(widget.newStatus == "Cancelled" ) rescheduleDropdown
...
class _DropdownExampleState extends State<DropdownExample> {
  ...
  // Fetch reschedule reasons from the API
  Future<void> fetchRescheduleReasons() async {
    try {
      ...
      final response = await dio.get('https://www.dashlogistics.dev/api/v1/employee/rescheduleReason');
      if (response.statusCode == 200) {
        setState(() {
          options = (response.data['data'] as Map<String, dynamic>).values.cast<String>().toList();
        });
      } else {
        throw Exception('Failed to load reschedule reasons');
      }
    } catch (e) {
      print(e);
    }
...
class _RescheduleDropdownState extends State<RescheduleDropdown> {
  ...
  // Fetch cancel reasons from the API
  Future<void> fetchCancelReasons() async {
    try {
      ...
      final response = await dio.get('https://www.dashlogistics.dev/api/v1/employee/cancelReason');
      if (response.statusCode == 200) {
        setState(() {
          options = (response.data['data'] as Map<String, dynamic>).values.cast<String>().toList();
        });
      } else {
        throw Exception('Failed to load cancel reasons');
      }
    } catch (e) {
      print(e);
    }
